...
local animacoes = {
	...
	{ nome = "copao", dict = "amb@code_human_wander_drinking@beer@male@base", anim = "static", prop = "j_copao", flag = 49, hand = 28422 },
	{ nome = "copao2", dict = "amb@world_human_drinking@beer@male@idle_a", anim = "idle_a", prop = "j_copao", flag = 49, hand = 28422 },
...
-- SYNCED PARTICLE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("wazeSyncParticle")
AddEventHandler("wazeSyncParticle",function(asset,index)
	if NetworkDoesNetworkIdExist(index) then
		local v = NetToPed(index)
		if DoesEntityExist(v) then
			if asset == "champagne" then
				vRP.PtfxThis("scr_ba_club")
				wazeParticleFxLoopedOnPedBone("scr_ba_club_champagne_spray",v,0.0,0.4,0.2,-50.0,0.0,0.0,GetPedBoneIndex(v,11816),2.0,false,false,false)
			elseif asset == "money" then
				vRP.PtfxThis("scr_xs_celebration")
				wazeParticleFxLoopedOnPedBone("scr_xs_money_rain",v,0.0,0.4,0.6,-50.0,0.0,0.0,GetPedBoneIndex(v,11816),1.0,false,false,false)
			elseif asset == "peeing" then
				vRP.PtfxThis("core")
				wazeParticleFxLoopedOnPedBone("ent_amb_peeing",v,0.0,0.35,-0.15,-140.0,0.0,0.0,GetPedBoneIndex(v,11816),2.5,false,false,false)
			elseif asset == "poo" then
				vRP.PtfxThis("scr_amb_chop")
				wazeParticleFxLoopedOnPedBone("ent_anim_dog_poo",v,0.0,0.0,-0.6,0.0,0.0,20.0,GetPedBoneIndex(v,11816),2.0,false,false,false)
				wazeParticleFxLoopedOnPedBone("ent_anim_dog_poo",v,0.0,0.0,-0.6,0.0,0.0,20.0,GetPedBoneIndex(v,11816),2.0,false,false,false)
			end
		end
	end
...
AddEventHandler('bong',function()
	...
	if not IsPedInAnyVehicle(ped) then
		...
		SetTimeout(9000,function()
			...
			wazeScreenEffect("DMT_flight",120000,false)
			Wait(120000)
			TriggerEvent("cancelando",false)
...
function GetClosestPlayer()
    ...
    for index,value in ipairs(players) do
        ...
        if (target ~= ply) then
            ...
            local distance = #(vec3(targetCoords["x"],targetCoords["y"],targetCoords["z"]) - vec3(plyCoords["x"],plyCoords["y"],plyCoords["z"]))
