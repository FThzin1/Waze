...
-- 			if #(vec3(v.x,v.y,v.z), - vec3(px,py,pz)) <= v.visible_distance then
-- 				sleep = 5
...
function tvRP.AdminSetStandBY(timer)
	standby = timer
end
    ...
    while true do
        local delay = 1000
            ...
            for k,v in pairs(areas) do
                local player_in = (#(vec3(v.x,v.y,v.z) - vec3(px,py,pz)) <= v.radius and math.abs(pz-v.z) <= v.height)
