...
vRP.prepare("waze/waze_sel_userid","SELECT * FROM vrp_user_ids WHERE identifier = @licenca")
...
Config.Language = {
	joining = "[Waze] Entrando...",
	connecting = "[Waze] Conectando...",
	err = "[Waze] Não foi possível identificar sua Steam ou Social Club.",
	_err = "[Waze] Você foi desconectado por demorar demais na fila.",
	pos = "[Waze] Você é o %d/%d da fila, aguarde sua conexão. Acesse nossa loja para obter prioridade na fila e outros benefícios para ajudar o servidor: https://five-m.store/Waze",
	connectingerr = "[Waze] Não foi possível adiciona-lo na fila.",
    wlonly = "[Waze] Você não está aprovado na whitelist.",
	banned = "[Waze] Seu passaporte foi revogado na cidade.",
	steam = "[Waze] Você precisa estar com a Steam aberta para conectar."
}
	...
	for k,v in ipairs(self.QueueList) do
		if tmp.priority then
			if not v.priority then
				print( '[FILA] ' .. (name or 'N/A' ) .. ' entrou na fila na posicao ' .. k .. ' (PRIORIDADE: ' .. tostring(tmp.priority) .. ')')
				_pos = k
			else
...
CreateThread(function()
	local function playerConnect(name,setKickReason,deferrals)
		...
		while banned == nil do Wait(1) end
		if banned then
			CancelEvent()
			return
		end
		...
		CreateThread(function()
			...
			while true do
				...
				local msg = string_format("Waze Evolved\n\n"..Config.Language.pos.."%s\nEvite punições, fique por dentro das regras de conduta.\nAtualizações frequentes, deixe sua sugestão em nosso discord.\nAcesse nossa loja para obter prioridade na fila e outros benefícios para ajudar o servidor",pos,Queue:GetSize(),dots)
...
RegisterCommand('fila',function(source,args,rawCommand)
	...
	if vRP.hasPermission(user_id,'admin.permissao') or vRP.hasPermission(user_id,'mod.permissao') or vRP.hasPermission(user_id,'suporte.permissao') then
		TriggerClientEvent("Notify",source,"importante",(#Queue.QueueList).." pessoas na fila da alfandega.")
	end
end)