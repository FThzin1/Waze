...
-- ROTEIRO DO ROUBO
-----------------------------------------------------------------------------------------------------------------------------------------
local registradora = 0
RegisterCommand('registradora', function(source, args, rawCmd)
	if func.checkPermission() then
		local ped = PlayerPedId()
		if GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_UNARMED") and not IsPedInAnyVehicle(ped) and registradora == 0 then
			registradora = 2
			for k,v in pairs(locais) do
				if #(vec3(v.x,v.y,v.z)  - GetEntityCoords(ped)) <= 1 and not andamento then

					if func.ubiBAERTGIKASdasa(v.id,v.x,v.y,v.z,v.h) then
						local res = exports["pd-safe"]:createSafe({math.random(0,99)})
						if res then
							func.iubAISUdBAOnIGoubda(v.id,v.x,v.y,v.z)
						else 
							TriggerEvent('Notify','negado', 'Você não conseguiu decifrar o cadeado corretamente.')
						end
...
CreateThread(function()
	while true do
		...
		if registradora > 0 then
			registradora = registradora - 1
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
