...
func = Tunnel.getInterface("vrp_atm")
	...
	while true do
		...
		if GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_UNARMED") and not IsPedInAnyVehicle(ped) then
			for k,v in pairs(locais) do
				if #(vec3(v.x,v.y,v.z) - GetEntityCoords(ped)) <= 1 and not andamento then
					timeDistance = 4
					...
					if IsControlJustPressed(0,47) and func.checkPermission() then
						if func.checkItem() then
							if func.checkRobbery(v.x,v.y,v.z) then
								...
								if not HasModelLoaded(dinheiro_prop) then
									...
									while contador >= 0 do 
										Wait(10)
										local ped = PlayerPedId()
										distance2 = #(vec3(v.x,v.y,v.z) - GetEntityCoords(ped))
										...
										if distance2 <= 1 then
											timeDistance = 4
											DrawText3Ds(x2,y2,z2-0.8,"~g~[G] ~w~PEGAR")
										...
										elseif #(vec3(v.x,v.y,v.z) - GetEntityCoords(ped)) > 15 then
											func.cancelRobbery()
							...
							TriggerEvent("Notify","negado","Você não possui uma <b>BOMBA CASEIRA</b>.")
						end
					end
				end
			end
...
AddEventHandler('blip:criar:caixaeletronico',function(x,y,z)
	if not DoesBlipExist(blip) then
		...
		AddTextComponentString("Roubo ao caixa eletrônico")
