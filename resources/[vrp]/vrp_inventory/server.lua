...
client = Tunnel.getInterface("vrp_inventory")
	...
	if data and data.inventory then
		for k,v in pairs(data.inventory) do
			if vRP.itemBodyList(k) then
				inventario[#inventario + 1] = { amount = parseInt(v.amount), name = vRP.itemNameList(k), index = vRP.itemIndexList(k), key = k, type = vRP.itemTypeList(k), peso = vRP.getItemWeight(k) }
			end
		end
			...
			if parseInt(amount) > 0 then
				if vRP.getInventoryWeight(nuser_id) + vRP.getItemWeight(itemName) * amount <= vRP.getInventoryMaxWeight(nuser_id) then
					if vRP.tryGetInventoryItem(user_id,itemName,amount) then
						...
						exports["system"]:sendLogs(user_id,{ webhook = "inventorySend", text = "Enviou x"..vRP.format(amount).." "..vRP.itemNameList(itemName).." para ("..nuser_id..") "..identitynu.name.." "..identitynu.firstname.."\nSET QUE ENVIOU: "..job.." | SET QUE RECEBEU: "..job2 })
				...
				for k,v in pairs(data.inventory) do
					if itemName == k then
						if vRP.getInventoryWeight(nuser_id) + vRP.getItemWeight(itemName) * parseInt(v.amount) <= vRP.getInventoryMaxWeight(nuser_id) then
							...
							exports["system"]:sendLogs(user_id,{ webhook = "inventorySend", text = "Enviou x"..vRP.format(parseInt(v.amount)).." "..vRP.itemNameList(itemName).." para ("..nuser_id..") "..identitynu.name.." "..identitynu.firstname.."\nSET QUE ENVIOU: "..job.." | SET QUE RECEBEU: "..job2 })
...
function vRPN.dropItem(itemName,amount)
	...
	if itemName then
		...
		if parseInt(amount) > 0 and vRP.tryGetInventoryItem(user_id,itemName,amount) then
			...
			exports["system"]:sendLogs(user_id,{ webhook = "inventoryDrop", text = "Destruiu x"..vRP.format(parseInt(amount)).." "..vRP.itemNameList(itemName) })
			TriggerClientEvent('Creative:Update',source,'updateMochila')
			return true
		else
			local data = vRP.getUserDataTable(user_id)
			for k,v in pairs(data.inventory) do
				if itemName == k then
					if vRP.tryGetInventoryItem(user_id,itemName,parseInt(v.amount)) then
						vRPclient._playAnim(source,true,{"pickup_object","pickup_low"},false)
						exports["system"]:sendLogs(user_id,{ webhook = "inventoryDrop", text = "Destruiu x"..vRP.format(parseInt(v.amount)).." "..vRP.itemNameList(itemName) })
						TriggerClientEvent('Creative:Update',source,'updateMochila')
						return true
					end
				end
...
function vRPN.useItem(itemName,type,ramount)
	...
	if user_id and ramount ~= nil and parseInt(ramount) >= 0 and not actived[user_id] and actived[user_id] == nil then
		if type == "usar" then
					...
					if bandagem[user_id] == 0 or not bandagem[user_id] then
						if vRP.tryGetInventoryItem(user_id,"bandagem",1) then
							...
							SetTimeout(20000,function()
								actived[user_id] = nil
								TriggerClientEvent('player:bandagem',source)
			...
			elseif itemName == "paraliancaouro" then
				TriggerEvent('Relacionamento:Casar', user_id)
			elseif itemName == "paraliancaprata" then
				TriggerEvent('Relacionamento:Namorar', user_id)
			...
			elseif itemName == "rebite" then
				if vRP.tryGetInventoryItem(user_id,"rebite",1) then
					...
					SetTimeout(10000,function()
						...
						TriggerClientEvent('inventory:BlockButtons', source, false)
						vRPclient._DeletarObjeto(source)
						TriggerClientEvent("Notify",source,'sucesso',"REBITE","Rebite utilizado com sucesso.",8000)
					end)
				end
			...
			elseif itemName == "presentedenatal" then
				local identity = vRP.getUserIdentity(user_id)
				if vRP.tryGetInventoryItem(user_id,"presentedenatal",1) then
					local rdm = math.random(200)
					if rdm <= 2 and rdm >= 1 then
					    TriggerClientEvent("Notify",source,"sucesso","Você ganhou $250.000 em dinheiro sujo.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasCommon", text = "Ganhou $250.000 em Dinheiro Sujo (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'dinheirosujo',250000)
					elseif rdm <= 4 and rdm >= 3 then
						exports["system"]:sendLogs(user_id,{ webhook = "xmasCommon", text = "Ganhou x1 FN FIVE-SEVEN e x70 M. FN FIVE-SEVEN (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'wbody|WEAPON_PISTOL_MK2',1)
						vRP.giveInventoryItem(user_id,'wammo|WEAPON_PISTOL_MK2',70)
					elseif rdm <= 6 and rdm >= 5 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou 1x G36 e 200x M. G36.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasLendary", text = "Ganhou 1x G36 e 200x M. G36 (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'wbody|WEAPON_SPECIALCARBINE_MK2',1)
						vRP.giveInventoryItem(user_id,'wammo|WEAPON_SPECIALCARBINE_MK2',200)
					elseif rdm <= 8 and rdm >= 7 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou $1.000.000 em dinheiro sujo.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasLendary", text = "Ganhou $1.000.000 em Dinheiro (Entrega automática)" })
						vRP.giveBankMoney(user_id,1000000)
					elseif rdm <= 10 and rdm >= 9 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou $1.500.000 em dinheiro sujo.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasLendary", text = "Ganhou $1.500.000 em Dinheiro Sujo (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'dinheirosujo',1500000)
					elseif rdm <= 25 and rdm >= 11 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou $100.000 em dinheiro.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasCommon", text = "Ganhou $100.000 em Dinheiro (Entrega automática)" })
						vRP.giveBankMoney(user_id,100000)
					elseif rdm <= 30 and rdm >= 26 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou $100.000 em dinheiro.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasCommon", text = "Ganhou $100.000 em Dinheiro (Entrega automática)" })
						vRP.giveBankMoney(user_id,100000)
					elseif rdm <= 33 and rdm >= 31 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou 1x AK-103 e x150 M. AK-103.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasRare", text = "Ganhou 1x AK-103 e x150 M. AK-103 (Entrega automática)" })
					...
					elseif rdm <= 36 and rdm >= 34 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou 1x SMG MK2 e x150 M. SMG MK2.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasRare", text = "Ganhou 1x SMG MK2 e x150 M. SMG MK2 (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'wbody|WEAPON_SMG_MK2',1)
						vRP.giveInventoryItem(user_id,'wammo|WEAPON_SMG_MK2',150)
					elseif rdm <= 40 and rdm >= 37 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou $250.000 em dinheiro.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasRare", text = "Ganhou $250.000 em Dinheiro (Entrega automática)" })
						vRP.giveBankMoney(user_id,250000)
					elseif rdm <= 46 and rdm >= 41 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou $500.000 em dinheiro sujo.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasRare", text = "Ganhou $500.000 em Dinheiro Sujo (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'dinheirosujo',500000)
					-- COMMON
					elseif rdm <= 49 and rdm >= 47 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou x1 TEC-9 e x70 M. TEC-9.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasCommon", text = "Ganhou x1 TEC-9 e x70 M. TEC-9 (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'wbody|WEAPON_MACHINEPISTOL',1)
						vRP.giveInventoryItem(user_id,'wammo|WEAPON_MACHINEPISTOL',70)
					elseif rdm <= 55 and rdm >= 49 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou x1 FN FIVE-SEVEN e x70 M. FN FIVE-SEVEN.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasCommon", text = "Ganhou x1 FN FIVE-SEVEN e x70 M. FN FIVE-SEVEN (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'wbody|WEAPON_PISTOL_MK2',1)
						vRP.giveInventoryItem(user_id,'wammo|WEAPON_PISTOL_MK2',70)
					elseif rdm <= 150 and rdm >= 51 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou $100.000 em dinheiro.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasCommon", text = "Ganhou $100.000 em Dinheiro (Entrega automática)" })
						vRP.giveBankMoney(user_id,100000)
					elseif rdm <= 200 and rdm >= 151 then
						TriggerClientEvent("Notify",source,"sucesso","Você ganhou $250.000 em dinheiro sujo.",8000)
						exports["system"]:sendLogs(user_id,{ webhook = "xmasCommon", text = "Ganhou $250.000 em Dinheiro Sujo (Entrega automática)" })
						vRP.giveInventoryItem(user_id,'dinheirosujo',250000)
                    end
		...
		elseif type == "equipar" then
			if vRP.tryGetInventoryItem(user_id,itemName,1) then
				...
				exports["system"]:sendLogs(user_id,{ webhook = "inventoryUse", text = "Equipou "..vRP.itemNameList(itemName) })
				TriggerClientEvent('Creative:Update',source,'updateMochila')
			end
		elseif type == "recarregar" then
			...
			if uweapons[weaponuse] then
				...
				if weaponusename == k then
					...
					exports["system"]:sendLogs(user_id,{ webhook = "inventoryUse", text = "Recarregou x"..parseInt(v.amount).." "..vRP.itemNameList(itemName) })
					TriggerClientEvent('Creative:Update',source,'updateMochila')
					end
				end
				end
