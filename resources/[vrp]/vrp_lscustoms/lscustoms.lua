...
local garages = { 
	...
	[5] = { locked = false, camera = { x = 1174.77, y = 2640.64, z = 37.76, heading = 348.96 }, driveout = {  x = 1174.77, y = 2640.64, z = 37.76, heading = 348.96 }, drivein = {  x = 1174.77, y = 2640.64, z = 37.76, heading = 348.96 }, outside = {  x = 1174.77, y = 2640.64, z = 37.76, heading = 348.96 }, inside = {  x = 1174.77, y = 2640.64, z = 37.76, heading = 348.96 }},
	[5] = { locked = false, camera = { x = 1175.0, y = 2639.27, z = 37.76, heading = 183.0 }, driveout = {  x = 1175.0, y = 2639.27, z = 37.76, heading = 183.0 }, drivein = {  x = 1175.0, y = 2639.27, z = 37.76, heading = 183.0 }, outside = {  x = 1175.0, y = 2639.27, z = 37.76, heading = 183.0 }, inside = {  x = 1175.0, y = 2639.27, z = 37.76, heading = 183.0 }}
}
...
local function AddMod(mod,parent,header,name,info,stock)
	...
	if (GetNumVehicleMods(veh,mod) ~= nil and GetNumVehicleMods(veh,mod) > 0) or mod == 18 or mod == 22 then
		...
		if LSC_Config.prices.mods[mod].wazeprice then
			local price = LSC_Config.prices.mods[mod].wazeprice
...
local function DriveInGarageAdmin()
	...
	if DoesEntityExist(veh) then
		...
		-- LSCMenu.categories:addSubMenu("waze","waze",nil,true)
		LSCMenu.categories.Lights:addSubMenu("waze","waze",nil,true)
		for n, mod in pairs(LSC_Config.prices.coresXenon) do
			local btn = LSCMenu.categories.Lights.waze:addPurchase(mod.name,mod.price)btn.corxenon = mod.corxenon
		end
				...
				if DoesEntityExist(veh) and GetPedInVehicleSeat(veh,-1) == ped and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh))) then
					for i,pos in ipairs(garages) do
						...
						if LSC_Config.oldenter then
							if #(vec3(outside.x,outside.y,outside.z) - GetEntityCoords(ped)) <= f(1) then
								DelayThread = 5
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,38) then
												if emP.checkPermission() then
													...
													TriggerEvent("Notify","negado","Apenas membros da <b>Mecânica</b> podem modificar os veículos.",8000)
												end
											else
												drawTxt("PRESSIONE  ~r~E~w~  PARA ACESSAR A ~y~MECANICA",4,0.5,0.93,0.50,255,255,255,180)
											end
										else
											drawTxt("~y~MECANICA~w~ OCUPADA NO MOMENTO",4,0.5,0.93,0.50,255,255,255,180)
										end
									else
										if IsControlJustPressed(1,38) then
											if emP.checkPermission() then
												...
												TriggerEvent("Notify","negado","Apenas os membros da <b>MECANICA</b> podem modificar os veículos.",8000)
											end
										else
											drawTxt("PRESSIONE  ~r~E~w~  PARA ACESSAR A ~y~MECANICA",4,0.5,0.93,0.50,255,255,255,180)
										end
							...
							if math.abs(GetEntityHeading(veh)-outside.heading) <= 90 and IsVehicleStopped(veh) and #(vec3(outside.x,outside.y,outside.z) - GetEntityCoords(ped)) <= f(5) then
								DelayThread = 5
								if not tableContains(LSC_Config.ModelBlacklist,GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
									if pos.locked then
										if not LSC_Config.lock then
...
function LSCMenu:onSelectedIndexChanged(name,button)
	...
	if m == "horn" then
		...
		if IsHornActive(veh) or IsControlPressed(1,86) then
			wazeVehicleHorn(veh,10000,"HELDDOWN",1)
		end
	end
end
