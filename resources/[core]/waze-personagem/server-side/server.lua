RegisterServerEvent("waze-personagem:finishedIntro")
RegisterServerEvent("waze-personagem:finishedCharacter")
...
function processSpawnController(source,statusSent,user_id)
	...
	elseif statusSent == 1 or statusSent == 0 then
		userlogin[user_id] = true
		TriggerClientEvent("waze-personagem:characterCreate",source)
	end
end

AddEventHandler("waze-personagem:finishedIntro", function()
	--print("finishedIntro")
	-- setUData spawnController = 1
	TriggerClientEvent("waze-personagem:characterCreate", source)
end)

AddEventHandler("waze-personagem:finishedCharacter", function(characterNome, characterSobrenome, characterAge, currentCharacterMode)
	local source = source
...
function doSpawnPlayer(source,user_id,firstspawn)
	TriggerClientEvent("waze-personagem:normalSpawn",source,firstspawn)
	TriggerEvent("waze-barbershop:init",user_id)
end