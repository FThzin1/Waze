...
function addMap(file, owningResource)
    ...
    local length = (#mapFiles[owningResource] + 1)
    mapFiles[owningResource][length] = file
end
...
function parseMap(file, owningResource)
    ...
    TriggerEvent('getMapDirectives', function(key, cb, undocb)
        env[key] = function(...)
            ...
            local length = (#undoCallbacks[owningResource] + 1)
            undoCallbacks[owningResource][length] = function()
                undocb(state)
            end
