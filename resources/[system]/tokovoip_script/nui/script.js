...
function checkPluginVersion() {
	if (isPluginVersionCorrect()) {
		...
		document.getElementById('pluginVersion').innerHTML = `Plugin version: <font color="blue">${voip.pluginVersion}</font> (Required: ${voip.minVersion})`;
...
function updateTokovoipInfo(msg) {
	document.getElementById('tokovoipInfo').style.fontSize = '11px';
	let screenMessage;
	...
	switch (voipStatus) {
		...
		case PLUGIN_INITIALIZING:
			...
			break;
		case WRONG_SERVER:
			msg = `Connected to the wrong TeamSpeak server, please join the server: <font color="#3677f0">${voip.plugin_data.TSServer}</font>`;
	...
	if (msg) {
		//document.getElementById('tokovoipInfo').innerHTML = `<font color="${color}">[VOIP] ${msg}</font>`;
...
function updateConfig(payload) {
	voip = payload;
	document.getElementById('TSServer').innerHTML = `TeamSpeak server: <font color="#ff0000">${voip.plugin_data.TSServer}</font>`;
	document.getElementById('TSChannel').innerHTML = `TeamSpeak channel: <font color="#ff0000">${(voip.plugin_data.TSChannelWait) ? voip.plugin_data.TSChannelWait : voip.plugin_data.TSChannel}</font>`;
